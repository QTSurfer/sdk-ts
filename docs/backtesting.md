...
`backtest` owns compile, prepare, execution, and polling. It returns the completed result, while
...
`handle.equityCurve(runIx, options?)`. Non-retained rows answer `404`. The shared API
...
the leaderboard; `handle.results({ order: 'natural' })` rereads every available row without rerunning;
`handle.sensitivity()` provides marginals and heatmaps.
