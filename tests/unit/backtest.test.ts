...
const getBacktestResult = vi.fn();
const cancelBacktest = vi.fn();
...
describe('backtest workflow', () => {
  ...
  it('throws QTSExecutionError when execution fails and calls cancelBacktest on abort', async () => {
    compileStrategy.mockResolvedValue(ok({ strategyId: 'strategy-sync' }));
    prepareBacktest.mockResolvedValue(ok({ jobId: 'prep-1' }));
    getPrepareStatus.mockResolvedValue(
      ok({ status: 'Completed', size: 1, completed: 1 }),
    ...
    // Failed != canceled, so cancelBacktest is NOT invoked
    expect(cancelBacktest).not.toHaveBeenCalled();
  });

  it('surfaces submission errors as the matching QTSError subclass', async () => {
    compileStrategy.mockResolvedValue(err({ code: 400, message: 'bad source' }));
  ...
  it('honors AbortSignal and triggers server-side cancelBacktest when aborted during execute', async () => {
    ...
    executeBacktest.mockResolvedValue(ok({ jobId: 'exec-abort' }));

    // Keep getBacktestResult returning "Started" so the poll loops forever
    ...
    cancelBacktest.mockResolvedValue(ok({ status: 'cancelling' }));
    ...
    expect(cancelBacktest).toHaveBeenCalledWith(
      expect.objectContaining({
